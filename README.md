...
- **音频律动**（本项目新增的全部内容）：
  ...
  | 谱重心 | 色相微调的第二输入 |
  | 节拍 + 音乐能量 | **神经脉冲（突触放电）**：高速光信号沿连线逐跳传递，带动量的路径规划（优先直行、少拐弯、不回头），所过连线双向线性渐变点亮，鼓点时成簇爆发，跑十几到几十段后渐隐 |
...
| **⚡ 系统混音** | `Visualizer`（会话 0 = 输出混音，Wallpaper Engine 同款通路） | **一次授权永久免弹窗**，日常使用首选。精度较低（8-bit 波形、约 20Hz 回调），做音律可视化足够 |
...
- [x] 神经脉冲突触放电（动量路径规划：优先直行少拐弯；连线双向线性渐变高亮）
- [x] BPM 锁定小节呼吸（检测到节拍后整网按 4 拍正弦胀缩）
- [x] ⚡ 系统混音模式（Visualizer 免弹窗通路，含静音门限）
